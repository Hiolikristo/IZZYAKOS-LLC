...
async function market(url,res){
  ...
  if(!key||!secret) return send(res,503,{error:'Authorized Alpaca market-data credentials are not configured. Synthetic fallback is disabled.',code:'REAL_FEED_NOT_CONFIGURED'});
  ...
  if(!allowed.has(symbol)||!timeframes.has(timeframe)) return send(res,400,{error:'Unsupported symbol or timeframe'});
  ...
  try{
    ...
    const bars=(barsJson.bars||[]).map(b=>({t:b.t,o:Number(b.o),h:Number(b.h),l:Number(b.l),c:Number(b.c),v:Number(b.v),n:Number(b.n||0),vw:Number(b.vw||0)})).sort((a,b)=>new Date(a.t)-new Date(b.t));
    return send(res,200,{provider:'Alpaca Market Data',feed,symbol,timeframe,bars,market:{is_open:!!clockJson.is_open,timestamp:clockJson.timestamp||new Date().toISOString(),next_open:clockJson.next_open||null,next_close:clockJson.next_close||null}});
  }catch(err){ return send(res,502,{error:'Market-data backend failure: '+err.message});}
}
...
}).listen(port,'0.0.0.0',()=>console.log(`KAVANTQ API listening on ${port}`));
